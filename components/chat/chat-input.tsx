// updateddddd file as per latest changes
"use client";
...
// updateddddddd file as per latest changes
...
export const ChatInput = ({ apiUrl, query, name, type }: ChatInputProps) => {
  ...
  // updatedd file as per latest changes
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const url = qs.stringifyUrl({
        url: apiUrl,
        query, // query means ?conditions
      });

      await axios.post(url, values); // values = message("content" here) we are writing to someone or in channel
